...
public class ShowActivityCodeAdapter extends RecyclerView.Adapter<ShowActivityCodeAdapter.ViewHolder> {
    ...
    public ShowActivityCodeAdapter(Context context, List<ProjectActivityType> listProject, boolean fromParent) {
                ...
                if (fromParent) {
                    ...
                    if (activityCodeList.size() > 0) {
                       // ProjectActivityRepo.updateViewActivity(String.valueOf(listProject.get(i).getId()),1);
                        ...
                        holder.childRV.setAdapter(new ShowActivityChildAdapter(context, activityCodeList));
                    ...
                    {
                        interfaceList.checkParentClick(listProject.get(i),i);
                    }

            }else {
                    interfaceList.checkItemCodeClick(listProject.get(i),i);
                }
...
public interface InterfaceList {
    void checkItemCodeClick(ProjectActivityType list, int position);

    void checkParentClick(ProjectActivityType list, int position);
